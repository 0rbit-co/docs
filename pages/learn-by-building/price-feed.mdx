...
- Log the message details in the `LOGS` table for debugging.
...
- `getTokenPrice(msg)`: The function that sends the latest price of a token from the `TOKEN_PRICES` table.
...
- `fetchPrice()`: The function that sends a GET Request to 0rbit process to fetch the token prices and update the `TOKEN_PRICES` table.
...
- `Handlers.utils.hasMatchingTag("Action", "Receive-Data-Feed")`: Checks if the message action tag is `Receive-Data-Feed`
- `receiveData(msg)`: The function that updates the token prices in the `TOKEN_PRICES` table.
...
```bash
aos --load 0rbit-Price-Feed.lua --cron 30-seconds
```
The above command will load the `0rbit-Price-Feed.lua` file in a new ao process and create a cron job running every 30 seconds.
