...
The `Handlers.utils.hasMatchingTag` is a function that requires a message argument, so you can drop this into any pattern argument on the Handlers module. The function compares the `Tag Name` and `Value`, if they are equal then it invokes the handle function.
...
The following code checks if the token exists in the `TOKEN_PRICES` table. If not, it initializes its price to 0 and sets its `coingecko_id`. Then, it sends a success message to `msg.From` with relevant tags. Otherwise, it sends an error message indicating that the token already exists.
